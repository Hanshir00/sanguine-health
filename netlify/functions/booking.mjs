  ...
  if (req.method === 'POST') {
    ...
    await sql`
      UPDATE booking_content SET
        ...
        step1_title=${step1_title}, step1_text=${step1_text},
        step2_title=${step2_title}, step2_text=${step2_text},
        step3_title=${step3_title}, step3_text=${step3_text},
        updated_at=NOW()
      WHERE id=1
    `;
    return new Response(JSON.stringify({ message: 'Booking section updated.' }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  }
